...
This section follows \cite{Serre}.
...
\begin{example}[Trivial representation]\marginnote[left]{Is it a repr. in ANY v.s.? For example if dimV=3, is triv.repr. mapping to 3 by 3 id matrix? Later decomposable to direct sum 3T.}
	...
	\end{align*}
	This could be extended to any vector space $W$ by mapping $g$ to the $k \times k$ identity matrix, where $k = \dim W$.
\end{example}
...
In conclusion, for $\Cyc_n$ we have found $n$ representations of degree 1, each mapping $g$ to a $n$th root of unity. 

\begin{example}[Degree 1 representations of $\Cyc_4$]
	For $n = 4$, the four fourth roots of unity are $1,i,-1$ and $-i$ and the four corresponding representations of $\Cyc_4$ are presented in Table~\ref{table:C4}.\marginnote{Same example as Sagan? Bad?? More examples n = 3,5?}
