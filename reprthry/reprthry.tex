...
%	\textit{Detta är syntesen av tre tidigare versioner. }
	...
	\begin{note}
		If the vector space $V$ is provided with a basis, then a matrix representation of $G$ may be found. A matrix representation is not canonical, ie it is dependent on the basis chosen for $V$. %Two representations $\rho(g)$ and $\tilde{\rho}(g)$ are similar if there is a transformation $f$ from $\GL(V)$ such that $\tilde{\rho}(g) = f \cdot \rho(g)$ for every $g$ in $G$.
	\end{note}
	...
	\paragraph{Tensor product of representations.} The tensor product $V \otimes W$ is a representation.
